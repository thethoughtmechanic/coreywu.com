...
const CognitiveExtinctionGrid = () => {
  ...
  useEffect(() => {
    ...
    // Assign featured items to matching indices
    const assigned = [];
    yearData.featured.forEach((item, i) => {
      const available = statusIndices[item.status].filter(idx => !assigned.includes(idx));
      if (available.length > 0) {
        const randomIdx = available[Math.floor(Math.random() * available.length)];
        assigned.push(randomIdx);
        item.index = randomIdx;
      }
    });

    setDots(newDots);
    setFeaturedIndices(assigned);
  }, [currentYear]);
  ...
  const handleDotHover = (index) => {
    const yearData = timelineData[years[currentYear]];
    const featured = yearData.featured.find(f => f.index === index);
    if (featured) {
