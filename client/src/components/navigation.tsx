...
export function Navigation() {
  ...
  return (
    <>
      ...
      <div className={cn(
        "fixed top-16 right-0 h-[calc(100vh-4rem)] w-80 bg-soft-black/98 backdrop-blur-md border-l border-warm-brown/30 transform transition-transform duration-300 ease-in-out z-40 md:hidden shadow-2xl",
        isMenuOpen ? "translate-x-0" : "translate-x-full"
      )}>
        <div className="flex flex-col p-6 space-y-6">
          <div className="text-lg font-medium text-cream mb-4">
            Navigation
          </div>
          {navItems.map((item) => (
            <Link
              ...
              onClick={closeMenu}
              className={cn(
                "text-lg text-cream/90 hover:text-cream hover:bg-warm-brown/20 transition-all duration-200 py-3 px-4 rounded-lg border-l-4 border-transparent hover:border-l-warm-brown/50",
                location === item.path && "border-l-warm-brown text-cream bg-warm-brown/30 font-medium"
              )}
